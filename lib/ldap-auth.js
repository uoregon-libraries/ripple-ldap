...
function presenterAuth(auth, cb) {
  ...
  LDAPAuth.bind(auth, function(err) {
    ...
    LDAPAuth.findLocalUser(localName, function(err, user) {
      ...
      client.search(LDAPAuth.config.baseDN, {filter: LDAPAuth.config.presenterFilter.replace(USER_ID, auth.user), scope: "sub"}, function(err, res) {
        if (err) {
        ...
        res.on('end', function(result) {
          ...
          LDAPAuth.importLocalUser(newUser, function(err, user) {
            if (err) {
              return callback(err, user);
            }
...
function clientAuth(auth, cb) {
  ...
  LDAPAuth.bind(auth, function(err) {
    ...
    client.search(LDAPAuth.config.baseDN, {filter: LDAPAuth.config.clientFilter.replace(USER_ID, auth.user), scope: "sub"}, function(err, res) {
      if (err) {
        return callback(err, null);
      }
...
function getLDAPUser(auth, filter, callback) {
  client.search(LDAPAuth.config.baseDN, {filter: filter.replace(USER_ID, auth.user), scope: "sub"}, function(err, res) {
    if (err) {
      console.log("Error trying to search user data: " + util.inspect(err));
      return callback(err, null);
    }
...
function bind(auth, callback) {
  ...
  client.bind(LDAPAuth.config.bindDNFormat.replace(USER_ID, auth.user), auth.password, function(err) {
    if (err) {
      return callback(err)
    }
...
function connect() {
  ...
  client = ldap.createClient({url: LDAPAuth.config.hostname});
